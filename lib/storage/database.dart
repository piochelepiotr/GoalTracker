...
class DBProvider {
  ...
  initDB() async {
    return await openDatabase(
        ...
        onCreate: (db, version) {
          ...
          batch.execute("CREATE TABLE tasks(id INTEGER PRIMARY KEY, name TEXT, goal_id INTEGER)");
          return batch.commit();
        },
        onUpgrade: (db, oldVersion, newVersion) {
          if (oldVersion == 1) {
            return db.execute("CREATE TABLE tasks(id INTEGER PRIMARY KEY, name TEXT, goal_id INTEGER)");
          }
          return null;
        },
        version: 2,
  ...
  Future<List<Goal>> goals() async {
    ...
    List<Goal> goals = goalsMaps.map((goalMap) => Goal.fromMap(goalMap)).toList();
    List<Task> tasks = tasksMaps.map((taskMap) => Task.fromMap(taskMap)).toList();
    tasks.forEach((task) => goals.firstWhere((goal) => goal.id == task.goalID).tasks.add(task));
    return goals;
  }
